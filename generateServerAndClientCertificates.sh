...
openssl req -new -x509 -days 3650 -subj "/emailAddress=ca@sensebox/C=DE/ST=NRW/L=Muenster/O=senseBox/OU=senseBoxDevOps/CN=sketches_ca" -keyout ca_key.pem -out ca_cert.pem
...
openssl req -new -subj "/emailAddress=server@sensebox/C=DE/ST=NRW/L=Muenster/O=senseBox/OU=senseBoxDevOps/CN=localhost/subjectAltName=DNS.1=sketches,DNS.2=sensebox-sketches" -key server_key.pem -out server_csr.pem
...
cat << EOF > server_env.yml
      SENSEBOX_SKETCHES_SERVER_CERT: |-
$(cat server_cert.pem | while read line; do
  echo "        $line"
done)
      SENSEBOX_SKETCHES_SERVER_KEY: |-
$(cat server_key.pem | while read line; do
  echo "        $line"
done)
      SENSEBOX_SKETCHES_CA_CERT: |-
$(cat ca_cert.pem | while read line; do
  echo "        $line"
done)
EOF
